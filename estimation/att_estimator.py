...
def derive_dynamics_linearization():
    ...
    f_euler_error = ca.Function("eulerB321_error", [q.param, qr.param], [euler_error], ["q", "qr"], ["euler"])

    q1 = q * (omega * dt).exp(lie.SO3Quat)

    f_exp_quat = ca.Function("exp_quat", [q.param, omega.param, dt], [q1.param], ["q", "omega", "dt"], ["q1"])
...
def simulate():
    ...
    gyro_sqrt_noise_power = np.deg2rad(2.8e-3)  # sqrt of gyro noise power  [rad/sqrt(hz)]
    ...
    P0 = (1.0 * np.eye(3)).reshape(-1)  # initial attitude covariance (right invariant lie algebra)
    ...
    x_est = ca2np(ca.DM(lie.SO3Quat.from_Euler(lie.SO3EulerB321.elem(ca.DM(euler_est_0))).param))
    P = P0
    t_mag_last = -2 * dt_mag
    t_accel_last = -2 * dt_accel

    def sim_mag(x):
        return ca2np(eqs["mag_g"](x, np.random.randn(3), mag_decl, mag_incl, mag_str, mag_noise_std_dev))
