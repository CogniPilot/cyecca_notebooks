...
from cyecca.util import count_ops, rk4
...
def integrate_rk4(x0, a_b, omega_b, g, dt, tf):
    ...
    for i in range(1, len(t_list)):
        x[i, :] = np.array(f_integrate_rk4(x[i - 1, :], a_b, omega_b, g, dt)).reshape(-1)
    return data
...
def integrate_mixed_invariant(x0, a_b, omega_b, g, dt, tf):
    ...
    x[0, :] = x0
    for i in range(1, len(t_list)):
        x[i, :] = np.array(f_strapdown_ins_propagation(x0, a_b, omega_b, g, t_list[i])).reshape(-1)
    return data
...
def comparison(name: str, a_b: np.array, omega_b: np.array):
    ...
    data_mixed = integrate_mixed_invariant(x0=x0, a_b=a_b, omega_b=omega_b, g=g, dt=dt, tf=tf)
    ...
    if True:
        ...
        h_rk4 = plt.plot(data_rk4["x"][:, 0], -data_rk4["x"][:, 2], "r-", linewidth=5, label="rk4")[0]
        h_mixed = plt.plot(data_mixed["x"][:, 0], -data_mixed["x"][:, 2], "y-", label="mixed")[0]
        plt.xlabel("t, sec")
        plt.legend([h_rk4, h_mixed], ["rk4", "mixed"])
        plt.grid()
...
def draw_graphs():
    ...
    draw_casadi(f_integrate_rk4(x0, a_b, omega_b, g, dt), filename="fig/f_rk4.png", width=400)
...
def comparison_tolerance(name: str, x0: np.array, a_b: np.array, omega_b: np.array):
    ...
    for step in rk4_steps:
        ...
        data_mixed = integrate_mixed_invariant(x0=x0, a_b=a_b, omega_b=omega_b, g=g, dt=step, tf=tf)
        ...
        plt.figure(2)
        plt.plot(data_rk4["x"][:, 0], data_rk4["x"][:, 1], label=f"step size: {step:10g}")

        plt.figure(3)
        plt.subplot(321)
        plt.plot(data_rk4["t"], data_rk4["x"][:, 6], label=f"roll rk4 step size: {step:10g}")
        plt.subplot(323)
        plt.plot(data_rk4["t"], data_rk4["x"][:, 7], label=f"pitch rk4 step size: {step:10g}")
        plt.subplot(325)
        plt.plot(data_rk4["t"], data_rk4["x"][:, 8], label=f"yaw rk4 step size: {step:10g}")
        plt.subplot(322)
        plt.plot(
            data_mixed["t"],
            data_mixed["x"][:, 6],
        ...
        plt.figure(4)
        plt.plot(data_rk4["t"], data_rk4["x"][:, 3], label=f"vx rk4 step size: {step:10g}")
        plt.plot(data_rk4["t"], data_rk4["x"][:, 4], label=f"vy rk4 step size: {step:10g}")
        plt.plot(
            data_mixed["t"],
            data_mixed["x"][:, 3],
        ...
        plt.figure(5)
        plt.plot(data_rk4["t"], data_rk4["x"][:, 0], label=f"px rk4 step size: {step:10g}")
        plt.plot(data_rk4["t"], data_rk4["x"][:, 0], label=f"py rk4 step size: {step:10g}")
        plt.plot(
            data_mixed["t"],
            data_mixed["x"][:, 0],
